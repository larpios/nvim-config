...
package.preload["ccc"] = function() return { setup = function() end } end
...
local module_path = "custom.ccc"
local success, err = pcall(require, module_path)
if not success then
    print("Failed to require " .. module_path .. ": " .. tostring(err))
    os.exit(1)
end

-- Find RgbHslCmykInput - it's local in ccc.lua but it's registered in ccc.setup
-- Wait, the module doesn't return anything. It calls ccc.setup.
-- We need to capture what's passed to ccc.setup.
...
-- Reload
package.loaded["custom.ccc"] = nil
require("custom.ccc")

local RgbHslCmykInput = captured_input
if not RgbHslCmykInput then
    print("Failed to capture RgbHslCmykInput")
    os.exit(1)
end
